...
4. 面接可能になった時点で「応募者を呼び出す」を押します。設定チャンネルへ本人メンションを1度だけ送ります。
5. Botが質問を表示するので、「回答メモを入力」から質問番号ごとに記録します。
6. 「面接を完了して投票開始」を押すと、STEP1とは別のチャンネルで面接投票を作成します。
7. 設定票数または期限で自動確定し、合格時だけ別の発表チャンネルへ設定文と本人メンションを送ります。
...
「募集設定」の書類合格ロールIDと「面接設定」の面接合格ロールID・呼出チャンネル・呼出文を変更できます。ロールIDが空欄の場合は、Discord上の `書類合格者` / `面接合格者` という名前から一意に解決します。
