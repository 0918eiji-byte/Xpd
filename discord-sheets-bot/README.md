...
「募集設定」の書類合格ロールIDと「面接設定」の面接合格ロールIDを変更できます。ロールIDが空欄の場合は、Discord上の `書類合格者` / `面接合格者` という名前から一意に解決します。途中で画面を閉じても、同じ面接官が `/mensetu` を再実行すると進行中の回答画面を再開できます。
